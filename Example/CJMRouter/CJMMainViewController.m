...
@interface CJMMainViewController ()

@end

@implementation CJMMainViewController
...
- (void)viewDidLoad {
    ...
    UIButton *button1 = [[UIButton alloc] initWithFrame:CGRectMake(0, 150, [UIScreen mainScreen].bounds.size.width, 40)];
    [self.view addSubview:button1];
    [button1 addTarget:self action:@selector(pressedErrorView:) forControlEvents:UIControlEventTouchUpInside];
    [button1 setTitle:@"点击跳转到错误页" forState:UIControlStateNormal];
...
- (void)setCjmr_paramters:(NSDictionary *)cjmr_paramters {
    [super setCjmr_paramters:cjmr_paramters];
    
    NSLog(@"CJMRouterParamters=%@", cjmr_paramters);
}
...
- (void)pressedErrorView:(UIButton *)sender {
    NSString *url = @"app_scheme://goto?open_type=native_page&page_id=00000";
    [self.navigationController cjmr_pushViewControllerWithUrlString:url paramters:nil animated:YES];
}
